...
const usuarioModel = require("../models/imovelModel");
...
class UsuarioController {
  ...
  create(req, res) {
    ...
    return retorno
      .then((result) =>
        res.status(201).send("usuario criada com sucesso!")
      )
      .catch((error) => res.status(400).json(error.message));
  }
  ...
  update(req, res) {
    ...
    return retorno
      .then((result) =>
        res.status(200).send("usuario atualizada com sucesso!")
      )
      .catch((error) => res.status(400).json(error.message));
  ...
  delete(req, res) {
    ...
    return retorno
      .then((result) =>
        res.status(200).send("usuario deletada com sucesso!")
      )
      .catch((error) => res.status(400).json(error.message));
  }
